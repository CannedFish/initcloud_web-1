(function ($, w, undefined) {
    ...
    function Paginate() {
        ...
        p.init = function (ft) { //这个函数要随时执行
            if (ft.options.paginate === true) {
                if ($(ft.table).data('page') === false) return;
                $(ft.table).bind({
                    'footable_initialized': function () {
                        // alert('footable_initialized');
                        ft.pageInfo = new pageInfo(ft);//创建对象分页
		...
		p.setupPaging = function(ft) { // 发起分页
			var $tbody = $(ft.table).find('> tbody');
            // console.log($tbody);
            // var $tbody = $('.pagination');
			p.createPages(ft, $tbody); //  创建分页。
			p.createNavigation(ft, $tbody); // 创建导航
        ...
        p.createPages = function (ft, tbody) {
            ...
            var pageCount = pages * info.pageSize; // 分页数量 2
            var page = [];
            var lastPage = [];
            info.pages = [];
            var rows = tbody.find('> tr:not(.footable-filtered,.footable-row-detail)'); //
            rows.each(function (i, row) {// 分页机制（i, row）
                page.push(row);
                if (i === pageCount - 1) {
    ...
    if(w.footable === undefined || w.footable === null){
         setTimeout(function () {
           // console.log(w.footable);
           w.footable.plugins.register(Paginate, defaults); //注册插件
        }, 1000);
    }
    // w.footable.plugins.register(Paginate, defaults); //注册插件
