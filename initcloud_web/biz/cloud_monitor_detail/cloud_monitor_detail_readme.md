...
1.各个文件的说明
   ...
   cloud_monitor_detail_ctrl.js: 具体处理各项前端请求的js代码，所有的操作从这里发起。

   cloud_monitor_detail.html: 具体模版 
...
3.部署应用
   ...
   d.添加sidebar
        ...
        比如:
         ...
         <li>
            <a href="#/cloud_monitor_detail/">
                <i class="fa fa-users"></i>
                <span class="title">
                    {[{ 'sidebar.cloud_monitor_detail' | i18next }]}
                </span>
   ...
   g.拷贝cloud_monitor_detail_ctrl.js 文件拷贝至如下目录：	

      /var/www/initcloud_web/initcloud_web/render/static/management/controllers/

   h. 拷贝cloud_monitor_detail.html 文件拷贝至如下目录：
   ...
   j.add app in settings
      ...
      比如应用的名称是cloud_monitor_detail,则添加biz.cloud_monitor_detail
...
INSTALLED_APPS = [
   ...
   k.migrate app
     ...
     ./makemigrations.sh cloud_monitor_detail 
