...
1.各个文件的说明
   ...
   phy_monitor_network_ctrl.js: 具体处理各项前端请求的js代码，所有的操作从这里发起。

   phy_monitor_network.html: 具体模版 
...
3.部署应用
   ...
   d.添加sidebar
        ...
        比如:
         ...
         <li>
            <a href="#/phy_monitor_network/">
                <i class="fa fa-users"></i>
                <span class="title">
                    {[{ 'sidebar.phy_monitor_network' | i18next }]}
                </span>
   ...
   g.拷贝phy_monitor_network_ctrl.js 文件拷贝至如下目录：	

      /var/www/initcloud_web/initcloud_web/render/static/management/controllers/

   h. 拷贝phy_monitor_network.html 文件拷贝至如下目录：
   ...
   j.add app in settings
      ...
      比如应用的名称是phy_monitor_network,则添加biz.phy_monitor_network
...
INSTALLED_APPS = [
   ...
   k.migrate app
     ...
     ./makemigrations.sh phy_monitor_network 
