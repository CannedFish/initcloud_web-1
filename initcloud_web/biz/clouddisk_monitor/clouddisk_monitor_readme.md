...
1.各个文件的说明
   ...
   clouddisk_monitor_ctrl.js: 具体处理各项前端请求的js代码，所有的操作从这里发起。

   clouddisk_monitor.html: 具体模版 
...
3.部署应用
   ...
   d.添加sidebar
        ...
        比如:
         ...
         <li>
            <a href="#/clouddisk_monitor/">
                <i class="fa fa-users"></i>
                <span class="title">
                    {[{ 'sidebar.clouddisk_monitor' | i18next }]}
                </span>
   ...
   g.拷贝clouddisk_monitor_ctrl.js 文件拷贝至如下目录：	

      /var/www/initcloud_web/initcloud_web/render/static/management/controllers/

   h. 拷贝clouddisk_monitor.html 文件拷贝至如下目录：
   ...
   j.add app in settings
      ...
      比如应用的名称是clouddisk_monitor,则添加biz.clouddisk_monitor
...
INSTALLED_APPS = [
   ...
   k.migrate app
     ...
     ./makemigrations.sh clouddisk_monitor 
