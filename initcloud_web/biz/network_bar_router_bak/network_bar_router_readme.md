...
1.各个文件的说明
   ...
   network_bar_router_ctrl.js: 具体处理各项前端请求的js代码，所有的操作从这里发起。

   network_bar_router.html: 具体模版 
...
3.部署应用
   ...
   d.添加sidebar
        ...
        比如:
         ...
         <li>
            <a href="#/network_bar_router/">
                <i class="fa fa-users"></i>
                <span class="title">
                    {[{ 'sidebar.network_bar_router' | i18next }]}
                </span>
   ...
   g.拷贝network_bar_router_ctrl.js 文件拷贝至如下目录：	

      /var/www/initcloud_web/initcloud_web/render/static/management/controllers/

   h. 拷贝network_bar_router.html 文件拷贝至如下目录：
   ...
   j.add app in settings
      ...
      比如应用的名称是network_bar_router,则添加biz.network_bar_router
...
INSTALLED_APPS = [
   ...
   k.migrate app
     ...
     ./makemigrations.sh network_bar_router 
