...
from cloud.cloud_utils import create_rc_manually,create_rc_v3_manually
import traceback
import random
import ipaddress
...
class Virtualmechine_BarList(generics.ListAPIView):
	...
	try:
	    ...
	    LOG.info("--------------- neutron floating ip -------------------")
            ...
            rc3 = create_rc_v3_manually(request)
            v3_conn = v3_client.Client(auth_url=rc3.get("auth_url"),
                                        username=rc3.get("username"),
                                        password=rc3.get("password"),
                                        tenant_name=rc3.get("tenant_name"),
                                        insecure=insecure,
                                        cacert=cacert,
                                        debug=settings.DEBUG)
            v3_tenants = v3_conn.projects.list()
            float_ip = 0
            for tenant in v3_tenants:
                float_quota = neutron.tenant_quota_get(rc, tenant.id)
                for quota in float_quota:
                    if quota.name == 'floatingip':
                        float_ip = float_ip + quota.limit
